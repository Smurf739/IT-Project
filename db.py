...
class API_db():
    ...
    def add_api_key(self, api_key):
        try:
            ...
            return {'error': 'Repeating the key'}

    def checkAPI(self, api_key):
        res = self.cursor.execute('''SELECT * FROM API WHERE api_key = ?''', (api_key,)).fetchone()
        return res
...
class TariffPlan:
    ...
    def get_tariff_settings(self, tariff_name):
        """Возвращает настройки для каждого тарифа"""
        tariff_settings = {
            "Бесплатный": {
                "max_requests": 5,
                "api_perm": False,
    ...
    def register_user(self, user_login, password):
        """Регистрация нового пользователя с бесплатным тарифом"""
        try:
    ...
    def check_and_increment_request(self, user_login):
        """Проверяет возможность выполнения запроса и увеличивает счетчик"""
        try:
    ...
    def check_expired_tariffs(self):
        """Проверяет и обновляет просроченные тарифы (резервная проверка)"""
        try:
            now = datetime.now()
            ...
            for user in expired_users:
                ...
                print(
                    f"{datetime.now()}: Тариф пользователя {user[0]} автоматически переведен на бесплатный (резервная проверка)")
    ...
    def get_tariff_features(self, user_login):
        """Возвращает доступные функции для тарифа пользователя"""
        try:
    ...
    def shutdown(self):
        """Останавливает планировщик и закрывает соединение с БД"""
        scheduler.shutdown()
        self.conn.close()
